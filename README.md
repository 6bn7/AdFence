...
| 配置不丢 | Service 被系统按 START_STICKY 重建时（省电策略杀进程后）会**重新读取用户配置**，避免你设的私有上游 DNS 静默回退到公共 DNS |
| 上游校验 | 上游 DNS 只接受 **IPv4 字面量**：填域名会让系统去解析它、而解析又走回本应用，形成自递归并拖慢全机 DNS |
| 删除失败安全 | 留副本（留样/回收站）**失败就跳过删除**，不硬删；回收站配额满时先淘汰最旧副本 |
| 副本存放 | 留样与回收站都在**应用私有目录**，外部存储路径在 Android ≤10 可被其他应用读取，不用于存放这些副本 |
| 丢包处理 | 触发并发背压时回 **SERVFAIL**（让客户端快速重试）而不是静默丢包；含非 ASCII 的合法 IDN 查询改为转义记录、不再拒绝 |
| 并发安全 | 日志写入加锁；规则集合与统计表均为并发安全实现并带容量上限 |

> 历史审计发现（第一轮 H-2/H-3/M-1~M-4/L-1~L-6、第二轮 N-1~N-9）的处理见 `assets/changelog.txt` 中 v2.7 / v2.8 条目与提交记录。
> **尚未实现**：上游 DoT/DoH 支持（当前为明文 UDP 53）。若你的设备原本使用加密 DNS，启用本应用后该段链路为明文——这是本项目的已知取舍，上游地址可在设置里换成你自己的解析器。

## 验证你下载的 APK

安全工具的安装包应该能被验证。用你手机上的 APK 或下载的 APK 跑：

```bash
apksigner verify --print-certs AdFence.apk | grep SHA-256
```

与下面公布的**发布指纹**对比（应完全一致）：

```
0F:A7:66:17:3D:16:8F:B1:06:FA:A0:5A:6E:66:FA:32:E2:88:E8:5F:C9:DD:EB:D8:31:11:B9:BD:21:1E:5E:88
```

> 指纹对应本仓库的发布密钥（保存在作者本机 + CI Secrets，**不在仓库里**）。
> 如果你自己构建，指纹会不同（用的是你自己生成的钥匙）——这是正常的，也说明没人能冒充你的构建。
> 若你拿到的 APK 指纹与此不符，**请不要安装**，并到 Issues 反馈。

## 发布新版本（给维护者）

1. **保住发布密钥**：`keystore/release.keystore` 与 `keystore/.pass`（本地）/ CI 里的 `ANDROID_KEYSTORE_BASE64` + `ANDROID_KEYSTORE_PASS`。**换钥匙 = 老用户无法升级。**
2. 改 `version.env`（版本号的唯一来源）→ 提交 → 打 tag `v<版本>` → push。
3. GitHub Actions 会自动构建并在该次运行的 Artifacts 里给出 APK；把它挂到 Release 上。
4. 若指纹有变化（换了钥匙），**同步更新 README 里公布的指纹**。

> CI 未配置 Secrets 时也能构建，但产物使用**一次性密钥**：仅适合自测，**不要发布**——用户无法升级、也无法核对签名。
